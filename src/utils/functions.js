...
//   const dateActual = !isUnixTime ? local : moment.unix(DateTimeStamp);
...
//   return moment(dateActual).format(DATE_FORMATS[format] || format);
// };
...
//     (a, b) =>
//       formatDate(a.datetime.timestamp) - formatDate(b.datetime.timestamp)
//   );
...
export const accessNestedArray = (arr) => {
  // Write your code here
  ...
  for (let i = 0; i < arrLength; i++) {
    ...
    rightToLeft += arr[arrLength - 1 - i][i];
  }
  // return absolute difference value.
  return Math.abs(leftToRight - rightToLeft);
};

export const arrayValueRatio = (arr) => {
  // Write your code here
  const elements = arr.length;
  const pos = arr.filter((number) => number > 0).length / elements;
  const neg = arr.filter((number) => number < 0).length / elements;
  const zero = arr.filter((number) => number === 0).length / elements;

  const results = `${pos.toFixed(6)}\n${neg.toFixed(6)}\n${zero.toFixed(6)}`;
  console.info(
    "🚀 ~ file: functions.js ~ line 490 ~ arrayValueRatio ~ results",
    results
  );
